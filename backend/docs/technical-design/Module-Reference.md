...
| **pharmacy** | tenant | `pharmacy/dispensings` | `apps/api/src/pharmacy/pharmacy-dispensing.controller.ts`, `pharmacy-dispensing.service.ts`, `pharmacy-dispensing-label-document.ts`, `pharmacy-dispensing-number-generator.service.ts`, `pharmacy-order-cancellation.subscriber.ts`, `dto/create-walk-in-sale.dto.ts`, `entities/pharmacy-dispensing.entity.ts` | Inventory, Orders, Billing, @hospital/pdf | Two dispensing paths: order-routed (create `Pending` from an `OrderItem`, `dispense` decrements inventory and completes the order item — billing charge capture fires via `ChargeCaptureSubscriber`, best-effort) and walk-in/OTC (`POST /pharmacy/dispensings/walk-in-sale`, no doctor's order — creates already-`Dispensed` and bills in one atomic call via `InvoicesService.captureChargeForWalkInPharmacySale`, fail-loud: an unpriced item or a billing failure rolls back the whole sale). `cancel`/`reverse` on either path; `reverse` additionally voids the walk-in charge (`voidWalkInPharmacySaleCharge`) since a walk-in sale is typically reversed before any payment is recorded. `GET /pharmacy/dispensings/pending-items` worklist (order-routed only); `GET :id/dispensing-label.pdf` label print (works for both paths, patient resolved from `orderItemId` or the dispensing's own `patientId`). Permissions `pharmacy.read`, `pharmacy.dispensing.create`, `pharmacy.dispensing.dispense` (walk-in sale reuses `dispensing.dispense`, since it's create+dispense in one call). Spec: `2026-08-06-pharmacy-dispensing-design.md`. |

### Finance

| Module | Level | Route prefix(es) | Key files | Depends on | Notable |
|---|---|---|---|---|---|
| **billing** | tenant | `billing/invoices`, `billing/settings`, `billing/deposits` | `apps/api/src/billing/invoices.controller.ts`, `deposits.controller.ts`, `billing-settings.controller.ts`, `invoices.service.ts`, `deposits.service.ts`, `charge-capture.subscriber.ts`, `gst-tax-split.util.ts`, `money.util.ts`, `entities/{invoice,invoice-item,payment,deposit,return,billing-settings,billing-sequence,numeric.transformer}.entity.ts` | Accounting | Invoices from charge capture: `ChargeCaptureSubscriber` watches `order_items` → `Completed` and calls `invoices.service.captureChargeForOrderItem` (best-effort; unpriced items skip; serialized per patient by advisory lock); the pharmacy walk-in-sale path calls `captureChargeForWalkInPharmacySale` directly instead (no order item to watch — fail-loud, not best-effort). Both funnel into a shared `postChargeCapture` tail. GST: every line splits CGST+SGST (same-state) or IGST (inter-state, no CGST/SGST) via `splitGstTax()`, decided by `InvoicesService.isInterStateSupply()` comparing `billing_settings.stateCode` against the patient's `patient_addresses.stateCode` — determined once and snapshotted on `Invoice.isInterStateSupply` at invoice-creation time, not recomputed per line (Development-Standards.md §141). Invoice statuses `Unpaid`/`PartiallyPaid`/`Paid`/`Cancelled`; invoice number is an integer per (INV prefix, April-start financial year) via `billing_sequences`; payment modes `Cash`/`Card`/`UPI`/`Cheque`/`Deposit`/`Insurance`; returns (credit notes) and invoice cancellation post contra-revenue journals (Sales Returns / Accounts Receivable). `POST /billing/invoices`, `POST .../charge-capture` (recovery), list/get, `PATCH :id/cancel`, `POST :id/payments`, `POST :id/returns`, `GET :id/invoice.pdf`; deposits `POST /billing/deposits`, `PATCH :id/refund`; settings `GET`/`PATCH /billing/settings` (note: gated by `master-data.manage`, not a `billing.*` permission). Money is decimal Postgres `numeric`(…,2) via `numeric.transformer.ts` (parseFloat on read) + `roundMoney` — not integer paise. Invoice/payment/deposit/return inserts archive `InvoiceCreated`/`PaymentRecorded`/`DepositReceived`/`InvoiceReturned` reporting events (via the outbox — see Development-Standards.md §140). Permissions `billing.manage`/`billing.read`. Spec: `2026-08-01-billing-design.md`. |
...
- **Migrations.** `database/migrations/index.ts` splits two immutable baselines: `PLATFORM_MIGRATIONS`
  (`0093-initial-platform-schema` + `0099` append) run once by `migrate.ts`; `TENANT_MIGRATIONS`
  (`0094-initial-tenant-schema` + `0095…0104` appends, currently through GST place-of-supply
  fields) run per tenant by provisioning and by `migrate-tenants.ts` (backfills existing tenant
  schemas; skips purged/missing schemas). History 0001–0092 was squashed into the two baselines on
  2026-08-27 (Development-Standards §108).
...
| `api:seed-demo-catalog` / `api:seed-demo-data` (`database/seed-demo-data.ts`) | Realistic demo-hospital dataset in the `demo` tenant: ward/beds, master catalogs, patients, appointments, full visit record (vitals + encounter), an admission, an order driven through Lab/Pharmacy/Radiology completion (exercising real charge capture into an invoice), employees and a payroll run, plus demo staff accounts (doctor/nurse/receptionist/pharmacist/billing/HR/helpdesk/auditor — see `70671fb`). Passwords env-overridable (`DEMO_*_PASSWORD`). Gates its whole business-data block off once the tenant has any patients — a re-run against an already-seeded tenant is a no-op. |
| `api:seed-demo-bulk-data` (`database/seed-demo-bulk-data.ts`) | Extra QA volume layered on top of the above, for the same `demo` tenant: 20 more patients (phone range `9811001001`-`9811001020`, reserved for this seeder), 20 appointments, 10 full visits/invoices with a deliberate spread of billing states (3 Unpaid, 3 PartiallyPaid, 2 Paid, 1 Paid-then-partially-Returned, 1 Cancelled — so accounts-role QA has real payment/return/cancel work to do, not just a read-only view), and 5 admissions (2 discharged). Idempotent per record (keyed on phone number / invoice existence / admission existence), not gated on "has any data" — safe to re-run repeatedly against a QA tenant that's already been used for manual testing, including recovering cleanly from an interrupted prior run (the billing-state application step re-checks each invoice's actual status before touching it, rather than only acting on invoices it just created). |
