...
| **audit** | hybrid | `audit` | `apps/api/src/audit/audit.controller.ts`, `audit.service.ts`, `persisting-audit-event-publisher.ts`, `audit-wiring.service.ts`, `entities/audit-record.entity.ts` | TenantContext, Database, @hospital/audit-emitter | Global module. Binds `AUDIT_EVENT_PUBLISHER` to `PersistingAuditEventPublisher`, which writes `AuditRecord` rows (`@Entity('audit_records')`, diff as `jsonb`) into the tenant schema on a dedicated connection (best-effort; never rolls back the business write). `GET /audit` search over the last 24 h default, filters on table/action/actor/recordId/correlationId, gated by `audit.read`. Specs: `2026-07-31-audit-service-persistence-design.md`, `2026-08-22-entity-audit-columns-design.md`. |
...
- **Second pool:** reporting archive writes use a dedicated `REPORTING_DATA_SOURCE`
  (`database/reporting-data-source.ts`, max 3, 2 s acquire timeout), never the business pool.
