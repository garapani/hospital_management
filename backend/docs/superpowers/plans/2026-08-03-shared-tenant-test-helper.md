...
6. Delete the local `function inTenant...` — it's now `ctx.inTenant`. If the local helper takes an explicit `tenantId: string` parameter (a two-arg `inTenant(tenantId, work)` form, used so one helper can serve multiple tenants), there is no direct replacement — map each call site by which tenant it targets instead: `inTenant(tenantId1, fn)` → `ctx.inTenant(fn)`, `inTenant(tenantId2, fn)` → `<childCtx>.inTenant(fn)` (rule 8's `createTenant()` context). If a *further* local helper (e.g. `makePatient(tenantId, ...)`) itself calls the two-arg `inTenant`, change its parameter from `tenantId: string` to the `TenantTestContext` itself (`makePatient(tenantCtx, ...)`, body becomes `tenantCtx.inTenant(...)`) and update call sites to pass `ctx`/`<childCtx>` instead of the old tenant-id string. (Learned from Task 5, commit 7b9d86a: mechanically the same idiom as `cross-tenant-login.integration-spec.ts`, just applied per-call-site across a whole file instead of through one login flow.)
...
9. If the original file has a hardcoded literal tenant-id string that identifies this file's own fixed tenant, convert every occurrence to `ctx.tenantId` (or `<childCtx>.tenantId` for a second/third tenant from `createTenant()`) — whichever of these two shapes applies:
   - **Fixture/header object declared as `const` at `describe()`-body scope** (e.g. `const adminHeaders = { 'x-tenant-id': 'test_foo', ... }`): convert the `const` to `let`, assign it inside `beforeAll` after `ctx` resolves — the tenant id is only known once the async `setupTenantTestContext()` resolves.
   - **Bare inline literal at a call site or assertion, scattered across one or more `it()` bodies** (e.g. `.set('x-tenant-id', 'test_controller')`, `expect(decoded['hospitalId']).toBe('test_auth')`): replace each literal directly with `ctx.tenantId` (in scope via the describe-level `let ctx`) — no intermediate object needed. (Learned from Task 3, commit 8ffb531: mechanically the same problem as the const-object case, just without a wrapper object; both are "this file's fixed single tenant id now has to track a dynamically-assigned value.")
   Do NOT touch a hardcoded literal that is an unrelated business-table key rather than a per-tenant-schema routing id (e.g. a `TenantsService.provisionTenant(...)`-style `hospitalId` business key, as opposed to `AccountsService.provisionTenantSchema`'s schema-routing tenant id) — see rule 12 below for how to tell these apart. (Learned from Task 2, commit a431351.)
10. For controller-style specs using `Test.createTestingModule(...).overrideProvider(DataSource).useValue(dataSource)`: replace `.useValue(dataSource)` with `.useValue(ctx.dataSource)`. If the file separately resolves `moduleRef.get(AccountsService)` or `moduleRef.get(TenantContextService)` solely for out-of-band fixture setup (not for wiring into the Nest DI graph, e.g. a middleware constructor argument), use `ctx.accountsService`/`ctx.tenantContext` for that fixture setup instead — same schema, same effect, keeps every file's out-of-band setup consistent. Keep DI-resolved instances only where genuinely needed for DI wiring.
   **Caveat (confirmed load-bearing in Task 4, `master-data-permission-gating.integration-spec.ts`):** if the fixture-setup call goes through a DI-resolved service `ctx` does NOT expose (any domain service other than `AccountsService` — e.g. `moduleRef.get(MasterDataService)`), do NOT swap that call's ambient `TenantContextService` to `ctx.tenantContext`. Each `TenantContextService` instance owns its own private `AsyncLocalStorage` (`libs/tenant-context/src/lib/tenant-context.service.ts`), so a DI-resolved service's internal `TenantConnectionService` only sees tenant context set via that *same* DI-resolved `TenantContextService` instance — setting context via `ctx.tenantContext`/`ctx.inTenant()` while the fixture call itself still uses a different, DI-resolved service would silently produce "No tenant context set" or write through the wrong instance. In this case, keep the matching DI-resolved `TenantContextService` (via `moduleRef.get(TenantContextService)`) and only substitute the tenant-id *literal* inside its `.run(...)` call (e.g. `'test_foo'` → `ctx.tenantId`), not the `TenantContextService` instance itself.
...
12. **Before assuming a per-file note's premise holds, verify which service is actually in play.** Some services have similarly-named methods that are functionally unrelated (e.g. `TenantsService.provisionTenant(...)`, which writes a row to the shared business `tenants` table, vs. `AccountsService.provisionTenantSchema(...)`, which creates a per-tenant Postgres schema). A per-file note mentioning "provisionTenantSchema" or implying `seedRbac`/`ctx.accountsService` involvement does not apply if the file's actual subject is the former, not the latter — check the file's imports and actual method calls before applying a note's suggested treatment. (Learned from Task 3, commit 8ffb531: `tenants.service.integration-spec.ts` never touches `AccountsService` at all.) This includes the inverse case: if a file calls `TenantsService.provisionTenant(...)` *purely* to mint a `hospitalId` string that it then feeds into `AccountsService.provisionTenantSchema(...)`, and nothing in the file (or the service under test) ever reads the `tenants` table row that call creates, drop the `TenantsService.provisionTenant(...)` call entirely — `ctx`'s own sequential tenant ids serve the same purpose with no vestigial business-table row. Verify by grepping the file and the service under test for any `TenantsService`/`tenants`-table reference before dropping it. (Learned from Task 5, commit 7b9d86a: `billing-entities`/`billing-settings.service`/`deposits.service`/`invoices.service` all had this exact vestigial call.)
