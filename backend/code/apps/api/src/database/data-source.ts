...
import { NursingTask, MedicationAdministration, ShiftHandoffNote } from '../nursing/entities/nursing.entity.js';
...
import { CashierShift } from '../billing/entities/cashier-shift.entity.js';
...
export function createDataSource(): DataSource {
  ...
  const ds = new DataSource({
    ...
    password: dbPassword ?? 'hospital_db_password',
    database: process.env['DB_DATABASE'] ?? 'hospital_db',
    entities: [Role, Permission, RolePermission, Package, Account, AccountRole, Tenant, AuditRecord, DepartmentCatalog, Department, Ward, Patient, PatientAddress, PatientKin, PatientSequence, Appointment, Vital, ClinicalNote, Diagnosis, Prescription, TriageEntry, Bed, Admission, BedTransfer, DischargeSummary, Order, OrderItem, BillingSettings, BillingSequence, Invoice, InvoiceItem, Payment, CashierShift, Deposit, Return, ReportingEvent, LabTestCategory, LabTest, LabTestComponent, LabRequisition, LabResult, RadiologyImagingType, RadiologyImagingItem, RadiologyRequisition, InventoryItemCategory, InventoryItemSubCategory, InventoryItem, InventoryVendor, PurchaseOrder, PurchaseOrderItem, StockBatch, StockBalance, StockTransaction, StockRequisition, StockRequisitionItem, PharmacyDispensing, Notification, FixedAssetCategory, FixedAsset, AssetDepreciationEntry, InsurancePayer, PatientPolicy, InsuranceClaim, LedgerAccount, JournalEntry, JournalLine, WardStockBalance, WardStockBatch, WardStockTransaction, NursingTask, MedicationAdministration, ShiftHandoffNote, OtSurgery, MaternityRecord, CssdInstrument, CssdSterilizationCycle, Employee, Payslip, FractionRule, FractionEntry, HelpdeskTicket, ReferralSource, PatientReferral, SsuCase, VaccinationRecord, Subscription, SubscriptionInvoice, TenantBranding, OutboxEvent],
