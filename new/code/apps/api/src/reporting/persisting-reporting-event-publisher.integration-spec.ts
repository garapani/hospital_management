...
describe('PersistingReportingEventPublisher (integration)', () => {
  ...
  afterAll(async () => {
    ...
    try {
      await queryRunner.query(`DROP SCHEMA IF EXISTS "tenant_${TEST_TENANT_ID_1}" CASCADE`);
      await queryRunner.query(`DROP SCHEMA IF EXISTS "tenant_${TEST_TENANT_ID_2}" CASCADE`);
    } finally {
  ...
  function inTenant<T>(tenantId: string, work: () => Promise<T>): Promise<T> {
    return tenantContextService.run({ tenantId, correlationId: 'test-correlation' }, work);
  }

  async function getEvents(tenantId: string, eventType: string) {
    return inTenant(tenantId, () =>
      tenantConnection.runInTenantSchema((manager) =>
        manager.getRepository(ReportingEvent).find({ where: { eventType }, order: { occurredAt: 'ASC' } }),
      ),
    );
  }
  ...
  it('does not create a reporting event for unmapped entities (Patient)', async () => {
    await inTenant(TEST_TENANT_ID_1, async () => {
      const beforeCount = await tenantConnection.runInTenantSchema((m) => m.getRepository(ReportingEvent).count());
      ...
      const afterCount = await tenantConnection.runInTenantSchema((m) => m.getRepository(ReportingEvent).count());
  ...
  it('captures all 6 mapped business events successfully', async () => {
    await inTenant(TEST_TENANT_ID_1, async () => {
      ...
      const ward = await masterDataService.createWard({ wardName: 'RW1', wardCode: 'RW1', wardType: 'General' });
      const bed1 = await masterDataService.createBed({ wardId: ward.id, bedNumber: 'R-B1' });
      const bed2 = await masterDataService.createBed({ wardId: ward.id, bedNumber: 'R-B2' });
      ...
      const order = await ordersService.create({
        patientId: patient.id,
        orderedBy: DOCTOR_ID,
        items: [{ itemType: 'Lab', itemDescription: 'CBC', priority: 'Routine' }],
      });
      ...
      const admittedEvents = await getEvents(TEST_TENANT_ID_1, 'PatientAdmitted');
      ...
      const transferEvents = await getEvents(TEST_TENANT_ID_1, 'BedTransferred');
      ...
      // Transfer
      expect(transferEvents[1].entityId).toBe(transferEvents[1].payload.bedTransferId);
      ...
      const paymentEvents = await getEvents(TEST_TENANT_ID_1, 'PaymentRecorded');
      ...
      const depositEvents = await getEvents(TEST_TENANT_ID_1, 'DepositReceived');
